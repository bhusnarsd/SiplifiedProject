...
const createSaralInfo2 = catchAsync(async (req, res) => {
  const saral = await saralInformation2Service.createSaralInfo2(req.body.scode, req.body);
...
const getSaralInfoBysaralId = catchAsync(async (req, res) => {
  ...
  const result = await saralInformation2Service.getSaralInfo1BySaralId(scode);
  res.send(result);
});
